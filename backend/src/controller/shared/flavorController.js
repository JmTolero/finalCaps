...
const getAllPublishedFlavors = async (req, res) => {
  try {
    const [flavors] = await pool.query(`
      ...
      GROUP BY f.flavor_id, f.flavor_name, f.flavor_description, f.image_url, f.store_status, f.created_at, f.vendor_id, f.sold_count, v.store_name, v.profile_image_url, f.price_small, f.price_medium, f.price_large
